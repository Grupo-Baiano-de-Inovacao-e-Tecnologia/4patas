...
function verificarSolicitacaoEmAnalise($conn, $animal_id) {
    $query = "SELECT COUNT(*) AS total FROM solicitacao_adocao WHERE animal_id = ? AND status = 'analise'";
...
function excluirAnimal($conn, $animal_id) {
...
if (isset($_POST['excluir_animal_id'])) {
    $animal_id = $_POST['excluir_animal_id'];

    if (verificarSolicitacaoEmAnalise($conn, $animal_id)) {
        echo "<script>alert('Não é possível excluir este animal enquanto houver uma solicitação de adoção em análise.'); window.location.href = 'pagina_abrigo.php';</script>";
    } else {
        if (excluirAnimal($conn, $animal_id)) {
...
$condicoes[] = "a.id NOT IN ($subquery)";
...
<body>
            ...
            <?php if (count($animais) > 0): ?>
                ...
                <div class="animal">
                    ...
                    <div class="info">
                        ...
                        <div class="acoes">
                            <button class="editar" onclick="redirectToEdit(<?php echo $animal->id; ?>)">Editar</button>
                            <form method="POST" style="display:inline;">
                                <input type="hidden" name="excluir_animal_id" value="<?php echo $animal->id; ?>">
                                <button class="excluir" onclick="return confirmDelete(<?php echo $animal->id; ?>)">Excluir</button>
                            </form>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
            <?php else: ?>
                <p class="mensagem-vazia">Não há nenhum animal cadastrado no sistema.</p>
            <?php endif; ?>
        </div>
        <div class="btn-adotados">            
            <a href="animais_adotados.php">ADOTADOS</a>
        </div>
    </div>
    ...
    <script>
        ...
        function redirectToEdit(id) {
            window.location.href = 'editar_animal.php?id=' + id;
        }
